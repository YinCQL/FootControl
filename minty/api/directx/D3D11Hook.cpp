...
LRESULT CALLBACK hWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	if (settings.f_ShowMenu.getValue()) {
		...
		if (!m_IsPrevCursorActive) {
			app::Cursor_set_visible(true, nullptr);
			m_IsPrevCursorActive = app::Cursor_get_visible(nullptr);
			app::Cursor_set_lockState(app::CursorLockMode__Enum::None, nullptr);
		}

	}
	else if (m_IsPrevCursorActive) {		
		app::Cursor_set_visible(false, nullptr);
		m_IsPrevCursorActive = app::Cursor_get_visible(nullptr);
		app::Cursor_set_lockState(app::CursorLockMode__Enum::Locked, nullptr);
	}
