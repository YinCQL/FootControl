...
int main() {
    ...
    std::filesystem::path dll_path = current_dir.value() / "FootJob.dll";

    if (!fs::is_regular_file(dll_path)) {
        printf("FootJob.dll not found\n");
        system("pause");
        return 0;
    }
    ...
    fs::path settings_path = fs::current_path() / "FootJob.json";
    ...
    if (!fs::exists(settings_path)) {
        ...
        if (settings_file.is_open()) {
            ...
            if (!fs::is_regular_file(exe_path)) {
                std::cout << "File path in FootJob.json invalid" << std::endl;
                ...
                ofn.lpstrFilter = "Executable\0Endfield_TBeta_OS.exe";
                ofn.nFilterIndex = 1;
                ofn.lpstrTitle = "Select Endfield game executable";
                ofn.Flags = OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR;

                if (GetOpenFileNameA(&ofn)) {
                    std::string(exe_path) = ofn.lpstrFile;
                    std::ofstream settings_file("FootJob.json", std::ios_base::out);
